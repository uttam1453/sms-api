...
public class JwtTokenUtil implements Serializable {
    ...
    public UserEntity.ACCOUNT_STATUS getAccountStatusFromToken(String token) {
        UserEntity.ACCOUNT_STATUS status = null;
        try {
            final Claims claims = getClaimsFromToken(token);
            status = (UserEntity.ACCOUNT_STATUS) claims.get(STATUS);
        } catch (Exception e) {
            LOGGER.error("Password parsing problem -", e);
    ...
    public String generateToken(UserDetails userDetails) {
        ...
        claims.put(STATUS, user.getAccountStatus());
    ...
    public Boolean validateToken(String token, UserDetails userDetails, HttpServletResponse servletResponse) {
        ...
        else if (user.getAccountStatus() != UserEntity.ACCOUNT_STATUS.ACTIVE)
            servletResponse.addHeader("ACCOUNT_STATUS", user.getAccountStatus().toString());
        return
                username.equals(user.getUsername())
                        && !isTokenExpired(token) && password.equals(user.getPassword())
                        && user.getDeviceId() == deviceId
                        && user.getAccountStatus() == UserEntity.ACCOUNT_STATUS.ACTIVE
                ;
    }
}