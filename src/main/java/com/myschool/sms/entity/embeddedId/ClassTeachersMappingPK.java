package com.myschool.sms.entity.embeddedId;

import java.io.Serializable;
import javax.persistence.*;

/**
 * The primary key class for the CLASS_TEACHERS_MAPPING database table.
 * 
 */
@Embeddable
public class ClassTeachersMappingPK implements Serializable {
	//default serial version id, required for serializable classes.
	private static final long serialVersionUID = 1L;

	@Column(name="class_sec_id", insertable=false, updatable=false, unique=true, nullable=false)
	private int classSecId;

	@Column(name="teachers_id", insertable=false, updatable=false, unique=true, nullable=false)
	private int teachersId;

	public ClassTeachersMappingPK() {
	}
	public int getClassSecId() {
		return this.classSecId;
	}
	public void setClassSecId(int classSecId) {
		this.classSecId = classSecId;
	}
	public int getTeachersId() {
		return this.teachersId;
	}
	public void setTeachersId(int teachersId) {
		this.teachersId = teachersId;
	}

	public boolean equals(Object other) {
		if (this == other) {
			return true;
		}
		if (!(other instanceof ClassTeachersMappingPK)) {
			return false;
		}
		ClassTeachersMappingPK castOther = (ClassTeachersMappingPK)other;
		return 
			(this.classSecId == castOther.classSecId)
			&& (this.teachersId == castOther.teachersId);
	}

	public int hashCode() {
		final int prime = 31;
		int hash = 17;
		hash = hash * prime + this.classSecId;
		hash = hash * prime + this.teachersId;
		
		return hash;
	}
}